function accuracy = Visual_testIFCN_PascalContext()
...
if save
    ...
    resultFolder = fullfile(baseDir, 'skip_prediction');
...
netFn{end+1} = sprintf('PascalContext_%d_Val/batch-ires8s-5x5-6layers-101-dag-1e-3-higher3/net-BN-val.mat', imageSize); 
...
%     prob = cell(numel(nets),1);
        ...
        hh=0;
for ii=1:474
    if ~isempty(net_.vars(ii).value)
        hh=hh+1;
        skip_prod{hh}=gather(net_.vars(ii).value);
        if ~ flip
            skip_prod{hh}=skip_prod{hh}(:,:,:,1);
        else
            skip_prod{hh}=(skip_prod{hh}(:,:,:,1)+fliplr(skip_prod{hh}(:,:,:,2)))/2;
        end
        [~,skippred{hh}] = max(skip_prod{hh}, [], 3);
        skippred{hh}=skippred{hh}+1;
%         draw_label_image( uint8(I0), skippred{ii}, gt, cmap, ['unlabeled';classes]);
%         fn = fullfile(resultFolder, [imgIds{i}, net_.vars(ii).name,'.png']);
%         h = gca;
%         F = getframe(h);
%         im = F.cdata;
%         imwrite(im, fn);
        fn = fullfile(resultFolder,'/',imgIds{i},'/', [imgIds{i},'_', net_.vars(ii).name, '.png']);
        if hh==1
        mkdir([resultFolder,'/',imgIds{i}]);
        end
        imwrite(skippred{hh}, cmap, fn, 'png');

    end
end
        prob = gather(net_.vars(end).value);
        if ~ flip
            prob = prob(:,:,:,1);
        else
            prob = (prob(:,:,:,1) + fliplr(prob(:,:,:,2))) / 2;
        end
    ...
    if save
        ...
        fn = fullfile(resultFolder,'/',imgIds{i},'/', [imgIds{i}, '.png']);
        h = gca;
        F = getframe(h);
        im = F.cdata;
        imwrite(im, fn);
...
for i = 1 : nNets
    ...
    net_ = net_.net;
    for ii=1:numel(net_.vars)
        if  strfind(net_.vars(ii).name,'classifier')
            net_.vars(ii).precious=1;
        end
    end
    net_.vars(400).precious=1;
    net_.vars(406).precious=1;
    net_.vars(455).precious=1;
    net_.vars(456).precious=1;
    net_.vars(473).precious=1;
