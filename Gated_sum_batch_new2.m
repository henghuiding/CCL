function [h, dzdw1, dzdw2] = Gated_sum_batch_new2(X, w1, w2, DzDy)
...
%         Dgate=zeros(k,batch,m,n);
    for ii=1:num
        ...
        Dgate{ii}=bsxfun(@times,w1(:,:,ii),bsxfun(@times,dzdy,x{ii}));
        Dgate{ii}=permute(Dgate{ii},[3,4,1,2]);
        
        
        dzdw2(ii,3)=sum(mean(mean(mean(bsxfun(@times,Dgate{ii},gates3{ii}),1),2),3),4);
        Dgates3{ii}=bsxfun(@times,Dgate{ii},w2(ii,3));
        hh(:,:,ii,:)=sum(bsxfun(@times,Dgates3{ii},gates3{ii}),3);
    ...
    Y = Y .* bsxfun(@minus, hh, sum(hh .* Y, 3)) ;
    for ii=1:num
        
        dzdw2(ii,1)=sum(mean(mean(mean(bsxfun(@times,Dgate{ii},gates1{ii}),1),2),3),4);
        Dgates1{ii}=bsxfun(@times,Dgate{ii},w2(ii,1));
        Dgates1{ii}=sum(bsxfun(@times,Dgates1{ii},gates1{ii}),3)+Y(:,:,ii,:);
        Dgates1{ii}=bsxfun(@times,Dgates1{ii},DD);
        htmp=D;
        htmp(ci1{ii})=1;
        htmp=bsxfun(@times,htmp,Dgates1{ii});
        
        dzdw2(ii,2)=sum(mean(mean(mean(bsxfun(@times,Dgate{ii},gates2{ii}),1),2),3),4);
        Dgates2{ii}=bsxfun(@times,Dgate{ii},w2(ii,2));
        Dgates2{ii}=sum(bsxfun(@times,Dgates2{ii},gates2{ii}),3);
